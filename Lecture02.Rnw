...
\title{GLM -- Lecture 2\\Adding a predictor}
...
\begin{framed}
{\bf Plot, plot, plot.} Take out time to learn to draw plots,
both to learn more about your data and to communicate your findings
in talks and papers.
This script will feature some basic examples, but for a better introduction,
take a look at the following resources:
\begin{itemize}
  \item {\it Data visualization: A practical introduction} by Kieran Healy.
  Available for free at \url{https://socviz.co/}.
  
  \item The tutorials available from my blog: \url{https://janhove.github.io/tags.html#tutorial-ref}.
\end{itemize}
\end{framed}
...
this decrease is roughly linear. By way of contrast, Figure \ref{fig:nichtlinear}
...
In what follows, we will apply the techniques learnt in the previous sections
...
\[
  \textrm{a specific outcome} = \textrm{communality between all outcomes (incl.\ link with \textsc{aoa})} + \textrm{discrepancy}.
\]

Specifically, we will model this communality in terms of a straight line that is
a function of \textsc{aoa}. Straight lines are parametrised in terms of 
...
\textsc{gjt} score, and $\varepsilon_i$ still refers to the $i$-th residual.
...
{\bf Beware of the Greek letter fallacy:} Both the model from Lecture 1 and the one in Equation
...
Of course, what we want is a more principled method.
...
so as to minimise the sum of the estimated residuals.
...
the pair of estimates that minimises $\sum_{i = 1}^n \widehat{\varepsilon_i}^2$.
...
the maths are fairly easy and old hat (pretty much same as in the proof in Lecture 1,
but in two dimensions rather than just one),
but they require some familiarity with first-year university mathematics.
...
if we are willing to make some assumptions about the residuals. As in Lecture 1,
we need to assume that the residuals are i.i.d. 
We already covered the `independence' part of this assumption.
The `identical' bit (i.e., the homoskedasticity assumption) can be illustrated
...
distribution of the residuals, more specifically the variance of this distribution, changes with $x$. 
...
<<fig.width = 6, fig.height = 1.4, echo = FALSE, fig.cap = "In all scatterplots, the spread of the data varies considerably according to the $x$ or $\\widehat{y}$ values.\\label{fig:heteroskedasticity}", message = FALSE, out.width = ".9\\textwidth">>=
x <- runif(100, 0, 100)
y_hat <- 0.3*x
...
So rather than writing the bootstrapped estimates in a vector,
we write them as rows in a two-column matrix:
<<cache = TRUE>>=
...
<<>>=
# apply(x, 2, function) applies that function to x by columns
apply(bs_b, 2, sd)
@
...
method or by referring to a standard normal distribution. So for 95\% confidence
intervals:
<<>>=
# percentile method
...
In step 2, we resampled the estimated residuals with replacement, but without
...
distribution.\footnote{Alternatives that do not assume homoskedasticity
...
participants with \textsc{aoa} values over 40, and similary for participants
with lower \textsc{aoa} values. This would correspond to the assumption
that the residuals for participants with high vs.\ low \textsc{aoa} values
are sampled from separate distributions. The code to run this bootstrap variation
would only be slightly more complicated.}

\subsection{The parametric bootstrap}
Alternatively, we could assume that the residuals are drawn from a
...
the homoskedasticity assumption: the variance of the normal distribution
($\sigma_{\varepsilon}^2$) is the same for all $i = 1, 2, \dots, n$.
...
for (i in 1:n_bootstrap) {
  # Steps 2 and 3
  d <- d |> 
    mutate(bs_outcome = Prediction + rnorm(n = n(), sd = sigma_aoa.lm))
  # Step 4
  bs_mod <- lm(bs_outcome ~ AOA, data = d)
  bs_b[i, ] <- coef(bs_mod)
}
...
for (j in 1:n_bootstrap) {
  bs_y_hat[j, ] <- bs_b[j, 1] + bs_b[j, 2]*new_aoa
}
@
...
<<out.width = "0.6\\textwidth", fig.width = 5, fig.height = 3, fig.cap = "Scatterplot with a 95\\% confidence band for the regression line.\\label{fig:bs_confidenceband}">>=
# Step 4
...
As long as we're assuming that the residuals are i.i.d.\ draws from
a normal distribution, we might as well estimate the standard errors
and compute the confidence intervals for the parameter estimates directly.
<<>>=
summary(aoa.lm)
confint(aoa.lm)
@
...
of the regression line some prespecified predictor values using
...
Equation \ref{eq:simple_regression_normal} is useful for getting a conceptual
grasp of the regression line. According to this equation, we assume that
the residuals are drawn i.i.d.\ from a normal distribution with mean 0
...
<<fig.width = 4, fig.height = 3, echo = FALSE, fig.cap = "If we assume that the residuals are i.i.d., then the regression line connects the conditional means for distribution of $y$ at different $x$-values. In this illustration, the residuals are all assumed to be normally distributed, but this is not a necessary assumption for making sense of the regression line. If the residuals aren't normally distributed, however, it's possible that the conditional means don't capture what's interesting about how $x$ and $y$ relate to each other.\\label{fig:conditionalmean}", out.width = ".8\\textwidth">>=
set.seed(1)
x <- runif(200)
dat <- data.frame(x = x,
...
\begin{proof}
According to the regression model, $y_i = \widehat{\beta}_0 + \widehat{\beta}_1x_i + \widehat{\varepsilon_i}$ for $i = 1, 2, \dots, n$. Hence we have
\[
  \frac{1}{n}\sum_{i = 1}^n y_i
  = \frac{1}{n}\sum_{i = 1}^n(\widehat{\beta}_0 + \widehat{\beta}_1x_i + \widehat{\varepsilon_i})
  = \frac{1}{n}n\widehat{\beta}_0 + \frac{1}{n}\widehat{\beta}_1\sum_{i=1}^n x_i + \frac{1}{n}\sum_{i=1}^n \widehat{\varepsilon_i}.
\]
We know that the mean of the estimated residuals is always zero when using \textsc{ols},
...
That said, even with densely sampled data it is still possible
...
$\widehat{\beta}_0$. The intercept represent the estimated conditional mean
...
or the median. Centring just mean subtracting this sensible value
...
homoskedasticity, normality) almost never hold literally. Instead of worrying
...
graphical data analyses or models capable of capturing nonlinearities (e.g.,
...
As always, plotting the data helps you and your readership from flying blind.
...
\begin{itemize}
  ...
  predictor variable is fixed at 0. Depending on your data and research question,
  this may not be relevant information, but you can translate your predictor
  data to make the intercept more meaningful.
...
\section*{Exercises}
\begin{enumerate}
  \item (Paper and pencil.) Let's fit another model on DeKeyser et al.'s data:
<<>>=
...
\item (With R.) The dataset \texttt{vanhove2014\_cognates.csv} contains a summary of some data I collected for my PhD thesis \citep{Vanhove2014}. 163 speakers of German were asked to translate 45 written and 45 spoken Swedish words into German. The columns \texttt{CorrectSpoken} and \texttt{CorrectWritten} contain the number of correct translations per participants in both modalities. The dataset \texttt{vanhove2014\_background.csv} contains some background information about the participants, including their performance on a handful cognitive tests.

Let's first combine these datasets. (In addition to learning how to visualise data, learning to summarise, transform, and combine datasets is incredibly useful, but unfortunately out of the scope of this lecture series. See \url{https://r4ds.had.co.nz/} if you're interested in learning more about these topics.)
<<>>=
