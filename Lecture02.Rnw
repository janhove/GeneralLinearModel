...
{\it Data visualization: A practical introduction}, which is 
available for free at \url{https://socviz.co/}.
You'll find some further resources on my website (\url{https://janhove.github.io}).
\parend
...
\mypar[The Greek letter fallacy]{Warning}
...
But to spell this out precisely, we would need some first-year university mathematics.
...
\texttt{FALSE}. Setting it to \texttt{TRUE} would plot a 95\% confidence band --
which is a concept we haven't yet discussed.

\mypar[Other optimality criteria]{Remark}
...
As a result, the code changes slightly. But the logic is the same.
<<cache = TRUE>>=
