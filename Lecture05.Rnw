...
\title{GLM -- Lecture 5\\Multiple predictors}
...
Conceptually, however, multiple regression models often researchers
...
But I think it's ultimately more useful to get used to the abstraction,
...
by fleshing out what the causal connections between the three varaibles are.
...
<<fig.width = 5, fig.height = 5, out.width="0.7\\textwidth", fig.cap="A scatterplot matrix of the three simulated variables. The numbers in the lower triangle are Pearson correlation coefficients and the number of data pairs they are based on. The trend lines in the scatterplots in the upper triangle are scatterplot smoothers. For more information, see \\url{https://janhove.github.io/reporting/2019/11/28/scatterplot-matrix}.\\label{fig:streudiagramm}">>=
...
\begin{itemize}
  ...
  then, according to the model, we'd expect the mean of their $y$ values to be $5.35 \pm 0.13$.
  
  \item If we were to take a large number of observations for which all $x$ values were 1,
  then, according to the model, their mean $y$ value is expected to be $0.20 \pm 0.08$ lower than that of the $x = 0$ group.
\end{itemize}
...
\begin{itemize}
  ...
  values were 0,
  then, according to the model, we'd expect the mean of their $y$ values to be $5.30 \pm 0.11$.
  
  \item If we were to take a large number of observations for which all $x$ values were 1
  and all $z$ values were 0,
  then, according to the model, their mean $y$ value is expected to be $0.52 \pm 0.12$ higher than that of the $x = 0, z = 0$ group.
...
For each simulated dataset and each mode, the estimated \texttt{x} parameter
is extracted.
...
\begin{itemize}
  ...
  \item Even if we did measure the confouders, we may not have measured
  them perfectly.
...
\begin{framed}
\noindent {\bf Don't pin your hopes on statistical tools to neutralise the effect of confounding variables.}
...
\paragraph{Exercise 1: Unknown confounders.}
In Figure \ref{fig:dag1_aufgabe1}, one confounders, $u$, was added to our 
...
\paragraph{Exercise 2: Measurement error on the confounder.}
...
\begin{enumerate}
  ...
  instead of $0.3$ for $\psi$ in Equation \ref{eq:dag1_aufgabe2}.
\end{enumerate}
...
<<message = FALSE, fig.width=1, fig.height=1, out.width=".2\\textwidth", echo = FALSE, fig.cap="In this scenario, the values of $x$ were assigned randomly and independenty of $z$. This is typical of experiments in which the participants are randomly assigned to the conditions.\\label{fig:dag2}">>=
dag2 <- dagitty("dag {
   z -> y
   x_r -> y
}")
...
will become clearer once we've simulated a couple of thousand dataset.
...
value than the estimates model 1 yields.
...
corresponds to the true value. But compared to the estimates by the first model,
...
So even if you don't actually care about $z$, it can be dividends to still collect
it and include it in your analysis.

Don't go overboard with this, though. One or two additional variables that you
strongly suspect to be correlated with the outcome may be useful, particularly
if they're not too strongly related to each other. But don't collect umpteen
...
causal effect of $x$ on $y$ is indeed such that a one-unit increases
...
that is, we should fit a simple regression model that does not include $z$.
If, however, we want to estimate the causal effect of $x$ on $y$ that is not
mediated by $z$, then we do need to control for $z$.
...
In the \textsc{dag} in Figure \ref{fig:dag5}, $z$ is also a posttreatment
...
@
In the scenario depicted in Figure \ref{fig:dag5}, including the additional
...
in Figure \ref{fig:dag4}. Here, $z$ is directly affected by $x$, but not
...
\begin{itemize}
  ...
  \item As the exercises will show, plots of the actual data may help prevent
...
\section*{Exercises}
\begin{enumerate}
  \item \citet{Vanhove2019} had 1,000 short French texts written by children
  rated by between 2 and 18 raters each on a 9-point scale for their lexical 
  diversity. For the purposes of this exercise, we're interested in modelling
  these human ratings in terms of the length of the text (the logarithmically
  transformed number of tokens, \texttt{log2\_ntokens}) and the type--token
...
@
  \begin{enumerate}
    \item (Paper and pencil.) For each claim, explain whether it is correct.
    ...
    \item (Paper and pencil.) Now explain once again what each of the three
    estimated model parameters literally means.
    
    \item (Paper and pencil.) According to this model, what mean rating
    would you expect for a text consisting of 64 tokens with a TTR of 0.7?
  \end{enumerate}
  
  \item (Paper and pencil.) Consider Figure \ref{fig:complexdag1}. Assume that you want to
...
@
  ...
  \item (Paper and pencil.) Same question for Figure \ref{fig:complexdag2}.
...
@
  ...
  \item (Paper and pencil.) Let's say that instead of merely observing and measuring all the
  variables in Figure \ref{fig:complexdag1}, we devise a randomised
  experiments in which we randomly assign the participants to values of $x$.
  \begin{enumerate}
