...
I used R version 4.3.0 when writing this script in 2023 and version 4.5.1
...
is any value $r$ such that there are equally as many $x_i$ values
that are at least as large as $r$ as there are that are at most as large as $r$.
...
Write $\overline y := 1/n \sum_{i=1}^n y_i$, i.e., the sample mean.
Note that
\begin{align*}
 \sum_{i=1}^{n} (y_i - \widehat{\beta}_0)^2
...
\end{align*}
The term $\sum_{i=1}^n (y_i - \overline y)^2$ doesn't depend on $\widehat{\beta}_0$,
whereas $n(\overline y - \widehat{\beta}_0)^2 \geq 0$ with equality if and only if $\widehat{\beta}_0 = \overline y$.
...
Observe, first, that the distribution of sample means has itself a mean, $\mu_{\overline x}$,
...
This again is true regardless of the shape of the distribution.
...
% @
% 
% That is, according to the normal approximation, there is a 2.5\% chance
% that a sample mean of three observations ends up below $0.1733$ and a 2.5\% chance
% that it ends up above $0.8267$.
% These `tail probabilities' match quite well how often we actually observe
% such extreme sample means.
% 
% You can change \texttt{n} to a different number and run the code again
% to see what things look like for different sample sizes.
% \parend
% 
...
Incidentally, you can look up the mean and variance of the distributions
...
The following commands computes the proportion of sample means lie in the interval
$[3.45, 3.55]$, and also estimates this proportion under the \textsc{clt}-based
normal approximation:
<<>>=
mean(sample_means >= 3.45 & sample_means <= 3.55)
pnorm(3.55, mean = mu, sd = sd_n) - pnorm(3.45, mean = mu, sd = sd_n)
@
Due to the discreteness of the population, it's completely impossible to find
a sample mean in the interval $[3.45, 3.55]$.

For a given distribution with distribution function $F$
and a number $\gamma \in (0,1)$,
the quantile function $F^{-1}(\gamma)$ outputs the smallest number
...
or according to the normal approximation (\texttt{qnnorm()}).
<<>>=
quantile(sample_means, probs = c(0.025, 0.975))
...
The first assumptions we'll make is that the errors, i.e.,
...
that this distribution has expectation (i.e., mean) 0 and finite variance.
...
about the value of one specific error, you can make more informed guesses about
the values of some other error, too.
...
(e.g., generalised least squares models, nonparametric bootstrapping; see \citealp{Zuur2009}, and \citealp{Hesterberg2014}, respectively, for accessible introductions).
...
What Figure \ref{fig:bootstrapdistributions} also shows is that none of the distributions of
...
Here, we use the more accurate standard error formula, but in case where it isn't available,
