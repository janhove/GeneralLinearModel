...
\title{GLM -- Lecture 4\\Interactions}
...
\begin{framed}
\noindent \textbf{Try out several plots.} 
...
The term $(x_{1,i} \cdot x_{2,i})$ may have surprised you, but it does the job:
...
takes on two values:
\begin{itemize}
 \item Luca (0) without Codeswitches (0): $x_{1,i} \cdot x_{2,i} = 0 \cdot 0 = 0$.
 \item Luca (0) with Codeswitches (1): $x_{1,i} \cdot x_{2,i} = 0 \cdot 1 = 0$.
 \item Dragan (1) without Codeswitches (0): $x_{1,i} \cdot x_{2,i} = 1 \cdot 0 = 0$.
 \item Dragan (1) with Codeswitches (1): $x_{1,i} \cdot x_{2,i} = 1 \cdot 1 = 1$.
\end{itemize}
...
rounding errors were correct. The brackets are superfluous but make the 
structure clearer:
\begin{itemize}
...
While we could recode the predictors in such a way that, for instance,
$\beta_1$ expresses the average difference in the ratings for Luca and Dragan
across both levels of code-switches and $\beta_2$ expresses the average difference in
the ratings for the recording with vs.\ the one without code-switches,
we won't do so here. That said, if you ever need to analyse a study with
a factorial design such as this, I recommend you read \citet{Schad2020}
or have a look at my blog post on recoding predictors.
...
strategy hat does \emph{not} work. This doomed strategy is to fit several 
...
predictors; see the blog entry \href{https://janhove.github.io/analysis/2017/06/26/continuous-interactions}{\textit{Interactions between continuous variables}}.
...
It is particularly important to appreciate this if the interaction in question
...
\section{Exercises}
\begin{enumerate}
  \item (Paper and pencil.) Consider the following fictitious experiment and
...
@
  \begin{enumerate}
    ...
    \item Explain what the estimated \texttt{Interaction} coefficient represents.
  \end{enumerate}
  
  \item (With R.) The main finding in \citet{Berthele2011b} was the 
  ...
  Tip: You can group the data by two variables by using \texttt{group\_by(variable1, variable2)}.
  
<<eval = FALSE, echo = FALSE>>=
d$Prediction <- predict(potential.lm)
d$Residual <- resid(potential.lm)

n_bootstrap <- 20000
bs_res <- matrix(ncol = 4, nrow = n_bootstrap)

for (i in 1:n_bootstrap) {
  d <- d |> 
    group_by(Dragan, WithCS) |> 
    mutate(bs_outcome = Prediction + sample(Residual, replace = TRUE))
  bs.lm <- lm(bs_outcome ~ Dragan + WithCS + DraganWithCS, data = d)
  bs_res[i, ] <- coef(bs.lm)
}

apply(bs_res, 2, sd)
apply(bs_res, 2, quantile, prob = c(0.05, 0.95))
@
