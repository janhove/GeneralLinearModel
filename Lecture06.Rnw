...
\title{The general\emph{ised} linear model -- Lecture 6\\The basics of logistic regression}
...
\end{quote}
Note that the consequences of Programme A is the same regardless of how its
...
can be retrieved easily, and uncertainty estimates for this difference can
be computed:
<<>>=
d <- d |> 
...
<<echo = FALSE, out.width=".5\\textwidth",fig.width = 0.6*7, fig.height = 0.6*5, fig.cap = "The logistic function onverts log-odds into probabilities.\\label{fig:logodds}">>=
library(tidyverse)
...
Generalised linear models can accommodate multiple predictors; see the exercises.
...
@
  \begin{enumerate}
    ...
    \item (Paper and pencil.) How would you answer Keysar et al.'s research question?
