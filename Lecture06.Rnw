...
\title{GLM -- Lecture 6\\The basics of logistic regression}
...
% of homoskedasticity and normality, bootstrapping techniques can be used
...
\end{quote}
For the other participants, the consequences of these programmes
...
\end{quote}
Note that the consequences of Programme A is the same regardless of how it's
consequences are framed, and the same goes for Programme B. Note furthermore
that in both programmes, 400 people are expected to die. The only difference
is that this is a certainty in Programme A and a probabilistic expectation
in Programme B.
...
Let's reconstructed the dataset:
<<>>=
library(tidyverse)
d <- tibble(
...
It's easy to compute the proportion of choices for Programme A (the sure option):
<<>>=
d |> 
  group_by(framing) |> 
...
can be retrieved easily, and uncertainty estimates for this differences can
be computed:
<<>>=
d <- d |> 
  mutate(sure_choice = ifelse(choice == "A", 1, 0),
         n.loss = ifelse(framing == "loss", 1, 0))
...
bootstrap that does not assume homoskedasticity, but we won't do so here.
...
that go outside the permissible range for probabilities. One solution to this
problem is to not work directly with proportions or probabilities
but to work with so-called {\bf log-odds} instead. To understand these,
...
\[
  \frac{109}{43} = \frac{0.7171}{1 - 0.7171} \approx 2.53.
\]
...
axis.\footnote{For $a > 1, c > 0$, if $a^b = c$, then $\log_a c = b$. The natural logarithm, written $\ln$, uses Euler's number, $e \approx 2.718$ for $a$. Note that since $a^0 = 1$, $\log_a 1 = \ln 1 = 0$. Note further that 
\[
  a^{\log_a c} = c.
\]} 
...
As these computations shows, a proportion or probability $p$ can be converted to 
...
<<echo = FALSE, out.width=".5\\textwidth",fig.width = 0.6*7, fig.height = 0.6*5, fig.cap = "Converting log-odds into probabilities.\\label{fig:logodds}">>=
library(tidyverse)
...
\[
  ...
  \end{cases}
\]
Our goal is to estimate the $p_i$s. We do so by transforming them to log-odds
and by modelling these log-odds in a linear model:
\begin{align}
...
which was briefly mentioned at the end of Lecture 1. In brief, we
...
where 
\[
  p_i = \frac{1}{+ \exp(-(\beta_0 + \beta_1x_i))}.
\]
We'll skip how this computation can be converted into a more computer-friendly
form. Figure \ref{fig:mle} shows that a $\widehat{\beta}_0$ value around $1$
...
@
  ...
  \item The difference between the log-odds of a sure choice when the \texttt{n.loss} value is 1 compare to when it is 0 is $-2.2$. This corresponds
...
We had already computed all these numbers by hand. The added value of the model is that it produces estimated standard errors
for the estimated $\beta$ parameters as well as (approximate) confidence intervals. Note that these are expressed in log-odds, too!
<<>>=
confint(tversky.glm)
@
We can straightforwardly express these confidence intervals on the odds scale by exponentiation:
<<>>=
...
Generalised linear models can accommodate several predictors; see the exercises.

\section{Continuous predictors}
You can also fit continuous predictors in the generalised linear model. By way of example, we'll take a look
...
words and their translation.\footnote{If we wanted to model the data of all participants,
...
it highlight a monotonous association between orthographic overlap and the participant's translation success.
...
We can straightforwardly fit these data in a logistic model:
<<>>=
...
To wrap off, let's draw a plot that shows how, according to the model,
...
ggplot(data = d,
  ...
  geom_point(shape = 1,
             position = position_jitter(width = 0, height = 0.02)) +
  stat_function(fun = fnc, args = list(model = translation.glm))
@
...
<<echo = FALSE>>=
d <- d |> 
  mutate(n.language = ifelse(d$language == "English", 0, 1),
         n.framing = ifelse(d$framing == "gain", 0, 1),
...
@
  \begin{enumerate}
    ...
    \item How would you answer Keysar et al.'s research question?
  \end{enumerate}
