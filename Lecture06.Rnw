...
\mypar[Linear probability model done better]{Remark}
  ...
  function is the identity (i.e., $\eta \mapsto \eta$) instead of the logit function.
...
\mypar[Confidence intervals using parametric bootstrapping]{Remark}
...
\mypar[$p$-values in the generalised linear model]{Remark}
...
and the fit the full model. We then count in how many cases the 
