...
\title{GLM -- Lecture 3\\Group differences}
...
deal with differences between three or more groups. Finally, we'll discuss
how data from factorial designs can be analysed.

\section{Differences between two groups}
Our first example in this lecture stems from the field of social psychology.
...
but I prefer to overlay the individual data points, too. Boxplots can sometimes
...
as individual data points. Moreover, the data points are jittered vertically so that
they don't overlap as much.

Using the functions provided by the tidyverse packages, we can easily
construct a table with the basic descriptive statistics for each of the
...
Now, we can use the newly created dummy variable just like the more or
...
skip straight to the estimated standard errors and confidence intervals
based on the assumption that the $\varepsilon_i$ were sampled i.i.d.\
from a normal distribution; in the exercises for this chapter, you'll learn
how to estimate standard errors and construct confidence intervals without
...
From Lecture 2, we also know that $ \widehat{\beta}_0 + \widehat{\beta_1}$
...
\[
  y_i = 3.53 - 0.006x_i + \widehat{\varepsilon}_i.
\]
(Due to rounding, this is the same equation as before. Note, however, that the actual estimate for the intercept is slightly different between the two models.) To obtain the estimate for the conditional mean for the treatment group, we use $x_i = 0.5$; to obtain the estimated conditional mean for the control group, we use $x_i = -0.5$:
\[
...
\begin{align*}
  ...
  &= \frac{(\widehat{\beta}_0 + 0.5\widehat{\beta}_1) + (\widehat{\beta}_0 - 0.5\widehat{\beta}_1)}{2} \\
...
to the average of the conditional means for both groups.
...
\begin{framed}
  ...
  For instance, if your two groups are called `English' and `Dutch', the Dutch group would be coded as 0 and would be incorporated into the intercept estimate. But if your two groups are called `Engels' and `Nederlands', the English group would be coded as 0.
...
than feminine. (German {\it Knie} is neuter, by the way.)
...
own dialect.
To this end, I devised an admittedly small experiment with three conditions:
\begin{itemize}
...
<<out.width = ".7\\textwidth", fig.width = 6*1.1, fig.height = 2.3*1.1, fig.cap = "\\label{fig:vanhove2018}">>=
...
The general linear model can easily deal with several
predictors. So we can add both dummy variables to the model.
...
condition. The estimate for strategy is the difference between the estimated
...
means for the information and the no information conditions. You can verify
...
bootstrap methods (see exercises). If we're comfortabe assuming that the
residuals were all drawn from the same normal distribution, we can use \texttt{summary()}
as before to obtain the estimated standard errors:
<<>>=
summary(mod.lm)$coefficients
@
Confidence intervals can be computed using \texttt{confint()}. The confidence interval
about the intercept estimate isn't of too much interest here, but it is computed by default:
<<>>=
...
explain how this can be accomplished; also see the entry on my blog \href{https://janhove.github.io/analysis/2020/05/05/contrast-coding}{\textit{Tutorial: Obtaining directly interpretable regression coefficients by recoding categorical predictors}}.
...
\section*{Exercises}
\begin{enumerate}
  \item (Paper and pencil.) Refer back to the \texttt{money.lm} model on page \pageref{model:money.lm}. How would the parameter estimates change if we coded the treatment group as 0 and the control group as 1?
  
  \item (Paper and pencil.) Refer back to the \texttt{money.lm2} model on page \pageref{model:money.lm2}. How would the parameter estimates change if we coded the treatment group as 1 and the control group as $-1$?
  
    \item (With R.) One of the other findings that \citet{Klein2014} sought to replicate
...
@
 Inspect the dataset by typing its name at the R prompt, or use \texttt{View(gambler)}.
  ...
  \item (With R.) In the analyses in this lecture, we estimated standard errors and constructed
  confidence intervals based on the assumption that the residuals were i.i.d.\ normal. We can
  check whether we obtain similar results when we make different assumptions.
  To that end, we can run a semi-parametric bootstrap in which the bootstrapped residuals
  in a given condition are only sampled from that distribution. This way, we don't
  assume that the residuals are drawn from a normal distribution, and we acknowledge
  the possibility that the residuals in different conditions are drawn from different distributions.
  
  The code below is similar to that of the previous semi-parametric bootstraps;
  the only thing that was added is the \texttt{group\_by(Condition)} call in line 19.
  This splits up the dataset into groups by \texttt{Condition} so that the 
  resampling of the \texttt{Residual} values in line 20 only happens within each condition.
