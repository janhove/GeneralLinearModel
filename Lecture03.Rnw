...
Uncertainty estimates can be obtained like in Lecture 2. Here, we'll
...
\mypar{Exercise}
Refer back to the \texttt{money.lm} model on page \pageref{model:money.lm}. 
...
\mypar{Exercise}
Refer back to the \texttt{money.lm2} model on page \pageref{model:money.lm2}. 
...
\mypar[Semi-parametric bootstrap without homoskedasticity assumption]{Exercise}
  ...
  be reassigned to another observation in the information condition, and similary
...
@
How could you adapt this code so that it works for the money priming example?
\parend
